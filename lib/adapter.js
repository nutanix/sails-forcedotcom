...
module.exports = (function () {
  ...
  var adapter = {
    ...
    upsert: function(connectionName, collectionName, options, values, cb) {
      log.verbose('upserting ' + collectionName)

      spawnConnection(connectionName)
        // Execute logic
        .then(function (connection) {
          connection.sobject(collectionName)
            .upsert(_.extend(options.where, values), function (err, result) {
              if (err || !result.success) {return cb(err)}
