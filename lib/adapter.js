  ...
  var adapter = {
    ...
    find: function(connectionName, collectionName, options, cb) {
      ...
      var collection = connections[connectionName].collections[collectionName]
        ...
        , criteria = _.omit(options.where, ['pick', 'omit'])
        // TODO: Eventually, these will be built into the waterline spec and
        // will no longer be included withing the `where` criteria.
        , toPick = options.where.pick
        , toOmit = options.where.omit
        // Unserialze the collection attributes so that `pick` and `omit` keys
        // can use the same format as the model definition.
        , attrs = collection._transformer.unserialize(collection.definition)
        , projection

      // Shim in required query params.
      criteria = queryShim(criteria, collectionName)

      // Always include the model primary key within the projection.
      if (toPick && !_.contains(toPick, pk)) {toPick.push(pk)}
      if (toOmit && _.contains(toOmit, pk))  {toOmit = _.without(toOmit, pk)}

      // Before executing the find, built the projection criteria and transform
      // the model attributes to utilize their respective column names.
      attrs = (toPick) ? _.pick(attrs, toPick) : attrs
      attrs = (toOmit) ? _.omit(attrs, toOmit) : attrs
      projection = _.keys(collection._transformer.serialize(attrs))
