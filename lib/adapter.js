...
module.exports = (function () {
  ...
  var adapter = {
    ...
    find: function(connectionName, options, cb) {
      ...
      // whereClause = queryShim(options.criteria.where, options.using);
      whereClause = parseClause(whereClause);
    ...
    update: function(connectionName, options, cb) {
      ...
      spawnConnection(connectionName)
        ...
        .then(function (connection) {
          connection.sobject(options.using)
            .update(_.extend(options.criteria.where, options.valuesToSet), function (err, result) {
              if (err || !result.success) {return cb(err)}

              cb(null, [_.omit(result, 'success', 'errors')])
            })
        })
        .fail(cb)
        .done();
  ...
  function queryShim(options, collectionName) {
    ...
    if (!_.includes(Object.keys(shimMap), collectionName)) { return options }
    ...
    _.mixin({
      deep: function (obj, mapper) {
        return mapper(_.mapValues(obj, function (v) {
          ...
          if(_.isArray(v)){
            return _.map(v, function(item){return _.deep(item, mapper)})
          }
          return v
        }));
      }
    });
    newObj = _.deep(original, function(x) {
      return _.mapKeys(x, function (val, key) {
          return replace.indexOf(key) !== -1 ? '$'+key : key
      });
    });
    return newObj
  }
