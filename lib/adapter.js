  ...
  var adapter = {
    ...
    find: function(connectionName, collectionName, options, cb) {
      ...
      if (collectionName === 'Attachment') {
        this.download(connectionName, collectionName, options, cb);
      } else {
    ...
    create: function(connectionName, collectionName, data, cb) {
      ...
      if (collectionName === 'Attachment') {
        this.upload(connectionName, collectionName, data, cb);
      } else {
        spawnConnection(connectionName)
          // Execute logic
          .then(function (connection) {
    ...
    // Download a file attachment from salesforce.
    download: function(connectionName, collectionName, options, cb) {
      ...
      .then(function(connection) {
        ...
        fs = require('fs')
        fileOut = fs.createReadStream('./test80.txt', readOptions)
        .on('close', function() { cb(null, results)})
        docId = '00P6000000CQJcDEAX'

        connection.sobject('Attachment').record(docId).blob('Body').pipe(fileOut)
        .on('end', function(query) {
            sails.log.silly('Download complete.')
            cb(null, results)
          })
          .on('error', function(err) {
            sails.log.silly('In error')
            console.dir(err)
          })
          .on('record', function(record) {
            results.push(record)
    ...
    // Upload a file attachment to salesforce.
    upload: function(connectionName, collectionName, options, cb) {
      // TODO: Implement attachment upload logic. Currently jsforce doesn't support
      // attachment uploads.
    },
