...
module.exports = (function () {
  ...
  var adapter = {
    ...
    count: function(connectionName, collectionName, cb) {
      log.verbose('counting ' + collectionName.using)
      var collection = connections[connectionName].collections[collectionName.using]
      var options = collectionName.criteria
      ...
      spawnConnection(connectionName)
        .then(function(connection) {
          connection.sobject(collectionName.using)
            .count(function(){
    ...
    find: function(connectionName, collectionName, cb) {
      log.verbose('finding ' + collectionName.using)
      var collection = connections[connectionName].collections[collectionName.using]
    ...
    create: function(connectionName, collectionName, cb) {
      ...
      spawnConnection(connectionName)
        ...
        .then(function (connection) {
          ...
          connection.sobject(collectionName.using).create(_.omit(collectionName.newRecord, ['CreatedDate', 'CaseNumber', 'Id', 'Status']), function(err, result) {
            ...
            if (collectionName.meta && collectionName.meta.fetch) return cb(null, _.omit(result, 'success', 'errors'))
